...
**Build note: built (2026-07-17), REVIEWED + HARDENED (2026-07-17, commit 4d862c6), GRAPH VIEW TUNED (2026-08-02).** An initial `trailmem/dashboard.py` exists (authored by Amit, wired into cli.py). Audit + fresh-install functional test PASS: loopback-only bind, shared store/ops service layer (no direct SQLite writes), zero CDN/external assets, trigger-based revisioned SSE with Last-Event-ID replay + reset-on-gap, core validation on all write flows, no hard-delete in UI. Hardening added during review: Host/Origin loopback validation on every request (blocks DNS rebinding + cross-site CSRF), scope check on edge removal, clean port-in-use error. Graph tuning added Graphify-like community-territory layout, direct-neighborhood focus, cross-community spring separation, and restrained auto labels; temp-DB runtime smoke rendered 209 nodes / 273 edges with 21 visible labels after stabilization. The dashboard is a first-party local feature, not a replacement for the six stdio MCP tools.
...
- Layout is cluster-organised (2026-07-20, graphify-viewer inspired; tuned 2026-08-02): each client-detected community, not each whole connected component, settles in its own packed circular territory. This matters because real memory graphs often become one connected component. Same-community links stay tighter, cross-community links are longer and weaker, and each node is pulled toward its own community centre via ForceAtlas2-flavoured client-side physics (degree-weighted repulsion, collision padding, spring links, and community gravity). The simulation freezes after stabilising; no CDN graph library is used (offline rule above).
