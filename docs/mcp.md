...
**`-P` is mandatory, not cosmetic (0.1.11+).** `-m` puts the launching host's cwd at `sys.path[0]`, so any agent whose workspace is a checkout of *this* project imports the working tree instead of the installed package — it silently runs uncommitted code, and because `content_hash` covers absolute paths, the memories it writes never dedupe against the installed copy's. Live-hit 2026-07-29: every registered host on the maintainer's machine had been running the source tree, not 0.1.9, for days. `hosts._util.safe_path_flag()` emits `-P` on 3.11+ and `-I` on 3.10 (where `-P` does not exist and would abort startup); `hook_python()` applies the same flag to every hook command line, which has the identical exposure. `test_phase0` asserts both, so the flag cannot silently regress.
...
Hook registration = per-host config (Claude Code settings.json, Kiro `~/.kiro/hooks/` — user-level, and `<workspace>/.kiro/hooks/` is merged with it so trailmem installs only the user-level file, Codex hooks.json).
