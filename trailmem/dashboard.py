...
(() => {
  ...
  // Graphify-style organisation: community clusters, not whole connected
  // components, get their own territory. A memory graph often becomes one
  // connected component, so component-level packing collapses into a hairball.
  function componentGroups(){computeCommunities();const groups=new Map();for(const node of state.nodes){const cid=state.communities.get(node.node_id)?.cid??node.node_id;if(!groups.has(cid))groups.set(cid,[]);groups.get(cid).push(node.node_id)}return[...groups.values()].sort((a,b)=>b.length-a.length)}
  function packComponents(groups){const placed=[],centers=new Map(),spacing=Math.max(110,state.settings.linkDistance);for(const members of groups){const radius=spacing*(1+Math.sqrt(members.length)*1.05);let x=0,y=0;if(placed.length){for(let step=1;step<3200;step++){const angle=step*2.399963229728653,dist=step*spacing*0.075,cx=Math.cos(angle)*dist,cy=Math.sin(angle)*dist;if(placed.every(c=>Math.hypot(cx-c.x,cy-c.y)>=c.r+radius+spacing*0.5)){x=cx;y=cy;break}}}placed.push({x,y,r:radius});for(const id of members)centers.set(id,{x,y,r:radius})}return centers}
  ...
  // Selection highlights the direct neighborhood only. Highlighting the full
  // transitive component makes one selected memory light up almost the graph.
  function connectedSet(id){const set=new Set([id]);for(const next of adjacency().get(id)||[])set.add(next);return set}
  ...
  function forceTick(nodes,edgeRefs){const spacing=Math.max(90,state.settings.linkDistance),size=spacing*1.4,buckets=new Map();nodes.forEach((node,index)=>{const p=state.positions.get(node.node_id),key=`${Math.floor(p.x/size)},${Math.floor(p.y/size)}`;if(!buckets.has(key))buckets.set(key,[]);buckets.get(key).push(index)});const repel=state.settings.charge*3.8;nodes.forEach((node,index)=>{const p=state.positions.get(node.node_id),cellX=Math.floor(p.x/size),cellY=Math.floor(p.y/size);for(let ox=-1;ox<=1;ox++)for(let oy=-1;oy<=1;oy++)for(const otherIndex of buckets.get(`${cellX+ox},${cellY+oy}`)||[]){if(otherIndex<=index)continue;const other=nodes[otherIndex],q=state.positions.get(other.node_id);let dx=q.x-p.x,dy=q.y-p.y,d2=dx*dx+dy*dy;if(d2<1e-4){dx=Math.random()-.5;dy=Math.random()-.5;d2=dx*dx+dy*dy}const d=Math.sqrt(d2),mass=Math.sqrt(((Number(node.edge_count)||0)+1)*((Number(other.edge_count)||0)+1)),minimum=nodeRadius(node)+nodeRadius(other)+24,push=repel*mass/d+(d<minimum?(minimum-d)*0.55:0),px=dx/d*push,py=dy/d*push;if(!state.fixed.has(node.node_id)){node.vx-=px;node.vy-=py}if(!state.fixed.has(other.node_id)){other.vx+=px;other.vy+=py}}const home=state.clusterCenters.get(node.node_id);if(home&&!state.fixed.has(node.node_id)){node.vx+=(home.x-p.x)*0.04;node.vy+=(home.y-p.y)*0.04}});for(const edge of edgeRefs){const source=edge.source,target=edge.target,a=state.positions.get(source.node_id),b=state.positions.get(target.node_id),dx=b.x-a.x||.01,dy=b.y-a.y||.01,d=Math.hypot(dx,dy)||.01,same=state.communities.get(source.node_id)?.cid===state.communities.get(target.node_id)?.cid,wanted=state.settings.linkDistance*(same?1:2.4),spring=same?.055:.014,pull=(d-wanted)*spring,px=dx/d*pull,py=dy/d*pull;if(!state.fixed.has(source.node_id)){source.vx+=px;source.vy+=py}if(!state.fixed.has(target.node_id)){target.vx-=px;target.vy-=py}}for(const node of nodes){if(state.fixed.has(node.node_id)){node.vx=0;node.vy=0;continue}const p=state.positions.get(node.node_id);node.vx*=.84;node.vy*=.84;const speed=Math.hypot(node.vx,node.vy),cap=speed>9?9/speed:1;p.x+=node.vx*cap;p.y+=node.vy*cap}}
  ...
  graphEnhancementStyle.textContent=`
    ...
    .graph-stage.labels-expanded .node-label{opacity:0}
    .graph-stage.labels-expanded .node.selected .node-label,
    .graph-stage.labels-expanded .node.hovered .node-label,
    .graph-stage.labels-expanded .node.neighbor .node-label,
    .graph-stage.labels-expanded .node.is-hub .node-label{opacity:1}
